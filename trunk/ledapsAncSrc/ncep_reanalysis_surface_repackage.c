...
int main(int argc,char **argv) {
	...
	int32 status,index,count,dims[MAX_VAR_DIMS],start[MAX_VAR_DIMS],edge[MAX_VAR_DIMS],stride[MAX_VAR_DIMS];
	int32 attr_index;
	...
	char *ptrchr,title[200],tmpstr[200];
	int i,write_metadata,base_date_available;
   ...
   char   vgroup_name[VGNAMELENMAX], vgroup_class[VGNAMELENMAX];	
	char list_of_sds[4][10]={"pres","pr_wtr","air"};
...
****/
	...
	/* check if base_date is available */
			if (!strcmp(name,"base_date"))
				base_date_available=1;
			/* Free buffer */
			free(buffer);
  		}
/****
...
/* check the content of the file */
   	for (index = 0; index < nb_datasets; index++) {
		...
		stride[0]=1;
		edge[0]=dim_sizes[0];
		status=SDreaddata(sds_id,&start[0],&stride[0],&edge[0],timebuff);
/*		printf("%d\n",status);
		printf("%f\n",timebuff[0]);
		printf("%f\n",timebuff[1]);*/
